...
const Explorer = () => {
  ...
  const render_selection_games = (selection) => {
    ...
    return games.map((game, index) => {
      return (
        <div key={game.id} className={`flex w-1/6 h-auto flex-col  ${(index !== 0) ? 'ml-5' : 'ml-0'}`} >
          <img className={"object-cover w-full rounded-lg h-86"} src={'http://localhost:8000' + game.background_image} alt="cover" />
    ...
    return genres.map((selection, index) => {
      return (
        <div className={"w-9/12 h-auto"}>
          <div key={index} className={"flex flex-col flex-1 h-auto mt-2 rounded-lg"}>
    ...
    <Scafold className={''}>
      ...
      <div className={"flex flex-col items-center w-full h-auto"}>
        <div className={"w-full h-auto"}>
          <div className={"flex flex-row w-full h-60 bg-arc-dark_1"}>
          </div>
        </div>
        <div className={"w-9/12 h-auto"}>
          <h1 className={"mt-5 text-4xl text-white"}>Stores</h1>
        ...
        <div className={"w-9/12 h-auto"}>
          <h1 className={"mt-5 text-4xl text-white"}>Platforms</h1>
          <div className={"flex flex-row w-full h-auto p-2 mt-2 rounded-lg bg-arc-dark_1"}>
            <div className={"flex items-center justify-center flex-1 h-24 bg-white rounded-lg"}>
